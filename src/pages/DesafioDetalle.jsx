    ...
    <div className="min-h-[calc(100dvh-5rem)] px-4 sm:px-6 lg:px-8 py-10 relative">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        <div className="flex flex-col gap-4">
          ...
          {challenge.devlights && (
                  <>
                    {challenge.devlights.actividades.map((actividad) => (
                      <Section
                        key={actividad.id}
                        icon={
                          actividad.numero === "1" ? BookOpen :
                          actividad.numero === "2" ? Lightbulb :
                          actividad.numero === "3" ? Star :
                          actividad.numero === "4" ? AlertCircle :
                          actividad.numero === "5" ? Wrench : BookOpen
                        }
                        title={`${actividad.numero}. ${actividad.titulo}`}
                        delay={0.2 + parseInt(actividad.numero) * 0.02}
                      >
                        <div className="space-y-4">

                          {/* Texto principal */}
                          {actividad.contenido && (
                            <p className="text-sm text-primary-600 leading-relaxed">{actividad.contenido}</p>
                          )}

                          {/* Subsecciones (A1) */}
                          {actividad.subsecciones?.length > 0 && (
                            <div className="space-y-2">
                              {actividad.subsecciones.map((sub, i) => (
                                <div key={i} className={`rounded-lg border-l-4 p-3 ${sub.color}`}>
                                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${sub.badgeColor} mb-2 inline-block`}>
                                    {sub.titulo}
                                  </span>
                                  <p className="text-sm text-primary-700">{sub.descripcion}</p>
                                </div>
                              ))}
                            </div>
                          )}
                          ...
                          {/* Cierre (A2) */}
                          {actividad.cierre && (
                            <p className="text-sm text-primary-700 leading-relaxed font-medium border-t border-primary-200 pt-3">
                              {actividad.cierre}
                            </p>
                          )}
                          ...
                          {/* Problemáticas (A4) */}
                          {actividad.problematicas && (
                            <div className="space-y-2">
                              <p className="text-xs font-bold text-primary-500 uppercase tracking-widest mb-2">
                                Problemáticas
                              </p>
                              {actividad.problematicas.map((p, i) => (
                                <div key={i} className={`rounded-lg border-l-4 p-3 ${p.color}`}>
                                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${p.badgeColor} mb-1 inline-block`}>
                                    {p.titulo}
                                  </span>
                                  <p className="text-sm text-primary-700">{p.descripcion}</p>
                                </div>
                              ))}
                            </div>
                          )}

                          {/* Gestión del cambio (A4) */}
                          {actividad.gestionCambio && (
                            <div className="rounded-xl border border-purple-200 bg-purple-50 p-4">
                              <p className="text-xs font-bold text-purple-500 uppercase tracking-widest mb-2">
                                Gestión del Cambio
                              </p>
                              <p className="text-sm text-primary-700 leading-relaxed">{actividad.gestionCambio}</p>
                            </div>
                          )}
                          ...
                          {actividad.aportes && (
                            <ul className="space-y-2">
                              {actividad.aportes.map((aporte, i) => (
